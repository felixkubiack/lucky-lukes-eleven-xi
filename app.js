...
/* vorhandene renderPolls-Funktion erweitern */
const oldRenderPolls=renderPolls;
...
renderPolls=function(){
  ...
  const cards=[...document.querySelectorAll('#polls .card')];

  state.polls.forEach((p,i)=>{
    const card=cards[i];
    if(!card)return;

    if(card.querySelector('.delete-poll-btn'))return;
    ...
    btn.className='btn red delete-poll-btn';
    btn.textContent='🗑 Umfrage löschen';
    btn.onclick=()=>deletePoll(p.id);
